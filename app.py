...
routes = {
    "България → Германия": ["София", "Белград", "Виена", "Мюнхен", "Хамбург"],
    "България → Италия": ["София", "Скопие", "Рим", "Флоренция", "Венеция"],
    "България → Франция": ["София", "Будапеща", "Прага", "Париж", "Лион"],
    "Балканска обиколка": ["София", "Скопие", "Тирана", "Дубровник", "Сараево"],
    "България → Испания": ["София", "Мадрид", "Барселона", "Севиля"],
    "България → Гърция": ["София", "Атина", "Солун", "Санторини"],
    "България → Холандия": ["София", "Амстердам", "Ротердам"],
    "България → Португалия": ["София", "Лисабон", "Порто"]
}
...
trip_type = st.sidebar.selectbox("Тип на пътуването", ["Културно", "Приключенско", "Гастрономическо", "Романтично"])
hotel_type = st.sidebar.radio("Тип хотел", ["Бюджетен", "Среден", "Луксозен"])
highlight_pref = st.sidebar.multiselect("Акценти", ["Музеи", "Плажове", "Ресторанти", "Нощен живот"])
...
if st.sidebar.button("🚀 Планирай пътуването"):
    ...
    for i, city in enumerate(cities):
        ...
        with st.expander(f"📍 {city}"):
            ...
            st.markdown(f"**Тип пътуване:** {trip_type}")
            st.markdown(f"**Хотел според тип:** {hotel_type}")
            st.markdown(f"**Предпочитани акценти:** {', '.join(highlight_pref)}")
    ...
    # ================== SUMMARY ==================
    distance = len(cities)-1 * DISTANCE_BETWEEN_CITIES
